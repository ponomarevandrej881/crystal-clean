...
<br>
<div class="services">
    @foreach($cards as $offers)
        <div class="service_card" id="{{$offers['id']}}">
            <picture><source srcset="img/{{$offers['img']}}.webp" type="image/webp"><img src="img/{{$offers['img']}}.jpg" width="100%" height="75%"></picture>
            <a class="ref_card" href="card?id={{$offers['id']}}"><p class="card_text">{{$offers['name']}} {{$offers['price']}} руб.*</p></a>
        </div>
    ...
    @endforeach
        ...
        @if($cards->hasPages())
            @if($cards->hasMorePages())
                <a class="b" href="{{$cards->previousPageUrl()}}">Предыдущая</a>
                <a class="b" href="{{$cards->nextPageUrl()}}">Следующяя</a>
            @else
                <a class="b" href="{{$cards->previousPageUrl()}}">Предыдущая</a>
            @endif
        @endif()
